...
// Build-only patched copy of record_linux 0.7.2. See pubspec.yaml for why this
// vendored copy exists. The ONLY differences from upstream 0.7.2 are the two
// members required by record_platform_interface 1.6.0:
//   * startStream(...)  — added (unsupported on Linux via fmedia).
//   * hasPermission(...) — gained the `request` named argument.
// Everything else is verbatim from upstream.
...
class RecordLinux extends RecordPlatform {
  ...
  // Added in record_platform_interface 1.6.0. fmedia-based Linux recording does
  // not support raw PCM streaming, so this is unsupported (and never invoked on
  // Android/web, where the platform-specific implementations are used instead).
  @override
  Future<Stream<Uint8List>> startStream(
    ...
    throw UnsupportedError(
      'startStream is not supported by record_linux (fmedia backend).',
    );
  }
